...
export async function GET(){
 ...
 try{
  ...
  const [l,d,v,b]=await Promise.all([
  ...
  return NextResponse.json({ok:true,stats:{...l.rows[0],...d.rows[0],alerts:Number(alerts.rows[0]?.count??0),visits:v,last_backup_at:b.rows[0]?.created_at??null}});
